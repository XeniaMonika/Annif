...
# 2. Map uris to mapping file - done
...
# 3. See how many keywords are mapped in % - done
...
def extract_bne_id_to_label_mappings(mapping_data):
    """Extract BNE id to Spanish label mappings from mapping data.

    Iterate through the nested lists and for any dict whose '@id' starts
    with 'https://datos.bne.es/resource/' extract the id after the last
    slash and the first value from
    'http://www.w3.org/2004/02/skos/core#prefLabel' (its '@value').
    Returns a list of dicts like {"XX525289": "Arte"}.
    """
    bne_mappings = []
    ...
    for nested_list in mapping_data:
        ...
        for item in nested_list:
            ...
            bne_uri = item.get("@id", "")
            if not isinstance(bne_uri, str) or not bne_uri.startswith("https://datos.bne.es/resource/"):
                continue

            spa_id = bne_uri.rsplit("/", 1)[-1]
            pref_labels = item.get("http://www.w3.org/2004/02/skos/core#prefLabel", [])
            if not isinstance(pref_labels, list) or not pref_labels:
                continue

            first_label = pref_labels[0]
            if not isinstance(first_label, dict):
                continue

            spa_label = first_label.get("@value", "")
            if not spa_id or not spa_label:
                continue

            bne_mappings.append({spa_id: spa_label})

    return bne_mappings


def add_spa_label_to_combined(combined_keywords, mappings_spa):
    """Add Spanish labels to combined keywords using BNE ID mappings.
    
    For each dict in combined_keywords where spa_id is not empty,
    look up the corresponding Spanish label from mappings_spa.
    """
    label_lookup = {}
    for entry in mappings_spa:
        if isinstance(entry, dict) and len(entry) == 1:
            spa_id, spa_label = next(iter(entry.items()))
            label_lookup[spa_id] = spa_label
    ...
    for item in combined_keywords:
        ...
        if isinstance(spa_id, str) and spa_id:
            spa_label = label_lookup.get(spa_id)
            if spa_label:
                item["spa_text"] = spa_label
...
def save_mapped_keyword_files(path_small, path_large, combined_keywords, output_folder):
    """Enrich the small and large keyword files with spa_id/spa_label, save all three outputs.

    For each item in path_small and path_large (shaped like
    {"id": "gnd/4055964-6", "keyword": "Spanien", "count": 6056}), add
    spa_id and spa_label looked up from combined_keywords by matching the
    "id" value against ger_id. Keywords with no Spanish match get empty
    strings for both new keys.

    Saves:
        keywords_mapped_small.json
        keywords_mapped_big.json
        keywords_mapped_both.json   (small + large merged, counts summed by id)
    """
    os.makedirs(output_folder, exist_ok=True)

    lookup = {}
    for entry in combined_keywords:
        if not isinstance(entry, dict):
            continue
        ger_id = entry.get("ger_id", "")
        if ger_id:
            lookup[ger_id] = (entry.get("spa_id", ""), entry.get("spa_text", ""))
    ...
    def enrich(path):
        ...
        for item in data:
            if not isinstance(item, dict):
                continue
            ger_id = item.get("id", "")
            spa_id, spa_label = lookup.get(ger_id, ("", ""))
            new_item = dict(item)
            new_item["spa_id"] = spa_id
            new_item["spa_label"] = spa_label
            enriched.append(new_item)
        return enriched

    enriched_small = enrich(path_small)
    enriched_large = enrich(path_large)

    with open(os.path.join(output_folder, "keywords_mapped_small.json"), "w", encoding="utf-8") as f:
        json.dump(enriched_small, f, ensure_ascii=False, indent=2)

    with open(os.path.join(output_folder, "keywords_mapped_big.json"), "w", encoding="utf-8") as f:
        json.dump(enriched_large, f, ensure_ascii=False, indent=2)

    combined_both = {}
    for item in enriched_small + enriched_large:
        key = item.get("id", "")
        if key in combined_both:
            combined_both[key]["count"] += item.get("count", 0)
        else:
            combined_both[key] = dict(item)

    combined_both_list = list(combined_both.values())

    with open(os.path.join(output_folder, "keywords_mapped_both.json"), "w", encoding="utf-8") as f:
        json.dump(combined_both_list, f, ensure_ascii=False, indent=2)

    return enriched_small, enriched_large, combined_both_list
...
mappings_ids = extract_gnd_to_bne_mappings(mapping_data)
combined_mapped = add_spa_id_to_combined(combined, mappings_ids)
mappings_spa = extract_bne_id_to_label_mappings(mapping_data)
stats = get_unmapped_spa_id_stats(combined_mapped)
combined_with_labels = add_spa_label_to_combined(combined_mapped, mappings_spa)
save_mapped_keyword_files(path_keywords_small, path_keywords_large, combined_with_labels, output_folder)
stats_after_labels = get_unmapped_spa_id_stats(combined_with_labels)
#print(combined[0:10])
...
print(stats_after_labels)  # Print the first 10 mappings for inspection